...
- **预览详细信息** — 预览内一键查看文件名、创建/应用时间、标签、所属集合，完整路径一键复制
- **重复图片检测** — 文件大小分桶 + 内容哈希两阶段查重，弹窗按组展示缩略图，支持逐组或全部清理（保留每组第一张），其余移入回收站
- **离线 HTML 画廊** — 一键把媒体库导出为 `wallspace-gallery` 文件夹：图片拷贝 + 单文件深色画廊页（瀑布流 + 灯箱，零外部依赖），打包即可分享离线浏览
- **相似图片检测** — aHash 感知哈希 + 汉明距离并查集分组（多线程解码），三档相似度阈值可选，同构图不同裁剪/滤镜也能归组
- **监视文件夹多目录** — 支持配置多个监视目录，新图片 30 秒内自动导入；旧单目录配置自动迁移，同名文件不冲突
- **大库性能优化** — 网格增量挂载 + 缩略图视口门控（IntersectionObserver，进入视口前不发图请求）；媒体库 store 浅响应式（shallowRef，数千张免深度代理）；预览改 canvas 按需绘制（放大时只画可见源区域，超大图内存峰值大幅降低）；更新检查改为设置内手动点击触发
- **每日精选分享卡** — 一键生成分享图卡（模糊背景 + 圆角主图 + 日期标题），竖版 1080×1620 / 横版 1620×1080 / 方形 1440×1440 三档规格，标题与署名可自定义（署名记忆），可复制到剪贴板或另存 JPEG；任意壁纸的预览页同样可用
- **导出为…** — 预览内一键裁剪导出：头像 1:1 / 社交竖图 4:5 / 手机 9:16 / 桌面 16:9（1080p / 2K / 4K）/ 带鱼屏 21:9 / 平板 4:3 预设，cover 模式支持拖拽取景、fit 模式完整显示；亮度 / 对比度 / 饱和度 / 模糊实时调整；JPG / PNG 双格式（PNG 的 fit 模式四边透明），自动记住上次导出设置；可加入媒体库或「另存为…」到任意位置
